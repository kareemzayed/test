<?php
	if($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(isset($_POST['signup'])) {
			...
			$rpass = filter_var($_POST['repassword'], FILTER_SANITIZE_STRING);
			...
			if(empty($errors)){
				...
				$stm = $conn->prepare("INSERT INTO user(name, email, password) VALUES('$name', '$email', '$pass')")->execute();
				$_SESSION['user'] = [
					'name' => $name,
					'email' => $email,
				];
			...
			if(empty($errors)) {
				$stm = $conn->prepare("SELECT * FROM user WHERE email = '$email'")->execute();
				$data = $stm->fetch();
				if(!$data) {
					$errors[] = "Wrong email !";
				}
